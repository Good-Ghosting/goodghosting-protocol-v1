...
* **[GoodGhostingPolygonCurve](./contracts/GoodGhostingPolygonCurve.sol)** is a separate contract, based on [GoodGhosting](./contracts/GoodGhosting.sol), but uses Curve instead of Aave as the yield generation strategy. This contract can integrate with two specific curve pools: 1) the aave pool (which accepts DAI, USDC and USDT tokens for deposit); and 2) the atricrypto3 pool (which accepts DAI, USDC, USDT, WETH and WBTC tokens for deposit). The yield generation strategy used in this contract consists of making the deposit into the contract, and then stake the LP (liquidity provider) tokens into Curve's gauge contract to earn wmatic and curve tokens as extra rewards. Note that, although the underlying Curve pools accept multiple "deposit tokens", our contract only accepts one deposit token. When the contract redeems the funds from Curve, the pool also withdraw only one token (the same as the deposit token). Reward tokens earned (wmatic and curve) aren't converted, but transferred to the winners when they withdraw their share from the pool. This contract is intended to be deployed to the **Polygon Network**.
...
To run test coverage: `yarn coverage` or `truffle run coverage`
...
- On another terminal window (from the root of the project directory), run `truffle test --network local-mainnet-fork` or `yarn test:fork:mainnet`
...
- On another terminal window (from the root of the project directory), run `truffle test --network local-polygon-vigil-fork` or `yarn test:fork:polygon:vigil`

- For running the whitelisted contract tests, On another terminal window (from the root of the project directory), run `truffle test --network local-polygon-whitelisted-vigil-fork` or `yarn test:fork:polygon-whitelisted:vigil`

##### Polygon Curve Pool
There is a different fork address to be used for this pool, the address is mentioned in the .env-sample file (search for "POLYGON CURVE Address").

- On another terminal window (from the root of the project directory), run `truffle test --network local-polygon-vigil-fork-curve` or `yarn test:fork:polygon::curve`
...
- On another terminal window (from the root of the project directory), run `truffle test --network local-celo-fork` or `yarn test:fork:celo`
...
- Deploy to kovan: `yarn deploy:kovan`
- Deploy to ropsten: `yarn deploy:ropsten`
- Deploy to mainnet (PRODUCTION): `yarn deploy:mainnet`
- Deploy to polygon (PRODUCTION): `yarn deploy:polygon`
- Deploy to polygon the whitelisted contract (PRODUCTION): `yarn deploy:polygon-whitelisted`
- Deploy to Celo Alfajores testnet: `yarn deploy:alfajores`
- Deploy to Celo Mainnet: `yarn deploy:celo`
...
- Deploy and Verify on kovan: `yarn deploy:verify:kovan`
- Deploy and Verify on ropsten: `yarn deploy:verify:ropsten`
- Deploy and Verify on mainnet (PRODUCTION): `yarn deploy:verify:mainnet`
- Deploy and Verify on polygon (PRODUCTION): `yarn deploy:verify:polygon`
- Deploy and Verify on polygon the whitelisted contract (PRODUCTION): `yarn deploy:verify:polygon-whitelisted`
- Deploy and Verify on Celo Alfajores testnet: `yarn deploy:verify:alfajores`
- - Deploy and Verify on Celo Mainnet: `yarn deploy:verify:celo`
...
```sh
> truffle compile --all && yarn ipfs && truffle deploy -f 2 --network kovan --reset
