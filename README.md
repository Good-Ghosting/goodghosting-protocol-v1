...
* **[GoodGhostingPolygonCurve](./contracts/GoodGhostingPolygonCurve.sol)** is an altogether separate contract since it can integrate with two curve pools, the aave pool or the atricrypto3 pool, additionally we get extra rewards by staking lp tokens into gauge contract to earn wmatic and cuve rewards, so in additional to the interest we get on providing liquidity we get these rewards too. This contract is intended to be deployed to the **Polygon Network**.
