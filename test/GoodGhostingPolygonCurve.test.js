...
contract("GoodGhostingPolygonCurve", (accounts) => {
    ...
    describe("when an user tries to join a game", async () => {
        it("reverts if the contract is paused", async () => {
            ...
            await truffleAssert.reverts(goodGhosting.joinGame(0,{ from: player1 }), "Pausable: paused");
        });
        ...
        it("reverts if the user tries to join after the first segment", async () => {
            ...
            await truffleAssert.reverts(goodGhosting.joinGame(0,{ from: player1 }), "Game has already started");
        });
        ...
        it("reverts if the user tries to join the game twice", async () => {
            ...
            await approveDaiToContract(player1);
            await truffleAssert.reverts(goodGhosting.joinGame(0,{ from: player1 }), "Cannot join the game more than once");
        });
        ...
        it("second player can join after cap spot (maxPlayersCount) is open by an early withdraw", async () => {
            ...
            await token.approve(contract.address, segmentPayment, { from: player2 });
            await truffleAssert.reverts(contract.joinGame(0, { from: player2 }), "Reached max quantity of players allowed");
            ...
            await token.approve(contract.address, segmentPayment, { from: nonPlayer });
            await truffleAssert.reverts(contract.joinGame(0,{ from: nonPlayer }), "Reached max quantity of players allowed");
    ...
    describe("when an user tries to make a deposit", async () => {
        it("reverts if the contract is paused", async () => {
            await goodGhosting.pause({ from: admin });
            await truffleAssert.reverts(goodGhosting.makeDeposit(0,{ from: player1 }), "Pausable: paused");
        });

        it("reverts if user didn't join the game", async () => {
            await approveDaiToContract(player1);
            await truffleAssert.reverts(goodGhosting.makeDeposit(0,{ from: player1 }), "Sender is not a player");
        });
        ...
        it("reverts if user tries to deposit during segment 0", async () => {
            ...
            await goodGhosting.joinGame(0,{ from: player1 });
            await approveDaiToContract(player1);
            await truffleAssert.reverts(goodGhosting.makeDeposit(0,{ from: player1 }), "Deposit available only between segment 1 and segment n-1 (penultimate)");
        });
        ...
        it("reverts if user is making a duplicated deposit for the same segment", async () => {
            ...
            await approveDaiToContract(player1);
            await truffleAssert.reverts(goodGhosting.makeDeposit(0,{ from: player1 }), "Player already paid current segment");
        });
        ...
        it("reverts if user forgot to deposit for previous segment", async () => {
            ...
            await timeMachine.advanceTime(weekInSecs * 2);
            await approveDaiToContract(player1);
            await truffleAssert.reverts(goodGhosting.makeDeposit(0,{ from: player1 }), "Player didn't pay the previous segment - game over!");
        });
    ...
    describe("when a user withdraws before the end of the game", async () => {
        it("reverts if the contract is paused", async () => {
            await goodGhosting.pause({ from: admin });
            await truffleAssert.reverts(goodGhosting.earlyWithdraw(0,{ from: player1 }), "Pausable: paused");
        });

        it("reverts if the game is completed", async () => {
            await advanceToEndOfGame();
            await truffleAssert.reverts(goodGhosting.earlyWithdraw(0,{ from: player1 }), "Game is already completed");
        });
        ...
        it("reverts if user tries to withdraw more than once", async () => {
            ...
            await timeMachine.advanceTimeAndBlock(weekInSecs);
            await goodGhosting.earlyWithdraw(0,{ from: player1 });
            await truffleAssert.reverts(goodGhosting.earlyWithdraw(0,{ from: player1 }), "Player has already withdrawn");
        });
        ...
        it("reverts if user tries to pay next segment after early withdraw", async () => {
            ...
            await timeMachine.advanceTimeAndBlock(weekInSecs);
            await approveDaiToContract(player1);
            await truffleAssert.reverts(goodGhosting.makeDeposit(0,{ from: player1 }), "Player already withdraw from game");
        });
    ...
    describe("when an user tries to withdraw", async () => {
        it("reverts if user tries to withdraw more than once", async () => {
            ...
            await truffleAssert.reverts(goodGhosting.withdraw(0,{ from: player1 }), "Player has already withdrawn");
        });
